...
describe('EscrowController', () => {
  ...
  describe('refund', () => {
    ...
    it('should call service refundEscrow with user ID and escrow ID', async () => {
      ...
      const response = await controller.refund(
        { user: { id: 'payer-1' } },
        dto,
      );
  ...
  describe('list', () => {
    it('should throw UnauthorizedException if no user', async () => {
      await expect(
        controller.list({ user: undefined }, '10', '0'),
      ).rejects.toThrow(UnauthorizedException);
    });
  ...
  describe('getById', () => {
    it('should throw UnauthorizedException if no user', async () => {
      await expect(
        controller.getById({ user: undefined }, 'escrow-1'),
      ).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('should call service getEscrow with user ID and escrow ID', async () => {
      ...
      expect(escrowService.getEscrow).toHaveBeenCalledWith(
        'payer-1',
        'escrow-1',
      );
      expect(response).toEqual(result);
