...
/**
 * Strict DOMPurify instance for Escrow Payments.
 *
 * ALLOWED_TAGS and ALLOWED_ATTR are empty so that only plain-text content
 * survives sanitisation. Escrow data (reasons, metadata, etc.) is
 * user-authored and must never contain rich HTML. Stripping everything
 * ensures that even if a row is poisoned via a compromised admin panel
 * or SQL injection, the client receives only clean values.
 */
...
/**
 * Deeply sanitise a value using the strict escrow DOMPurify config.
 *
 * Rules:
 ...
 */
export function sanitiseEscrowData<T>(value: T): T {
  if (typeof value === 'string') {
    return strictPurify.sanitize(value) as unknown as T;
  }

  if (Array.isArray(value)) {
    return value.map((item) => sanitiseEscrowData(item)) as unknown as T;
  }
  ...
  if (value !== null && typeof value === 'object') {
    ...
    for (const [key, val] of Object.entries(
      ...
      sanitised[key] = sanitiseEscrowData(val);
    }
