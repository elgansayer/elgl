...
describe('EscrowService', () => {
  ...
  // ── exponential backoff retry (HTTP 429) ─────────────────────

  describe('exponential backoff retry for HTTP 429', () => {
    const escrowRecord = {
      id: 'escrow-1',
      sender_id: 'sender-1',
      receiver_id: 'receiver-1',
    ...
    it('should retry on 429 errors and succeed eventually', async () => {
      const rateLimitError = { code: '429', message: 'Too Many Requests' };

      // getEscrow uses a single .single() call, easiest to test retry
      mockQueryBuilder.single
        .mockRejectedValueOnce(rateLimitError)
        .mockRejectedValueOnce(rateLimitError)
        .mockResolvedValueOnce({ data: escrowRecord, error: null });
      ...
      expect(result).toEqual(escrowRecord);
    ...
    it('should throw non-429 errors immediately without retrying', async () => {
      ...
      mockQueryBuilder.single.mockRejectedValueOnce(dbError);

      await expect(service.getEscrow('escrow-1')).rejects.toThrow(
    ...
    it('should throw after exhausting all retries on 429', async () => {
      ...
      mockQueryBuilder.single.mockRejectedValue(rateLimitError);
      ...
      await expect(service.getEscrow('escrow-1')).rejects.toEqual(
        rateLimitError,
      );

      // Default is 3 maxRetries, so 1 initial + 3 retries = 4 calls
      expect(mockQueryBuilder.single).toHaveBeenCalledTimes(4);
