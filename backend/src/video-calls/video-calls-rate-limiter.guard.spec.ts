...
import {
  ...
  VideoCallsRateLimitOptions,
} from './video-calls-rate-limiter.guard';
import { SupabaseService } from '../supabase/supabase.service';
import { ExecutionContext } from '@nestjs/common';

describe('VideoCallsRateLimiterGuard', () => {
  let guard: VideoCallsRateLimiterGuard;
  ...
  beforeEach(() => {
    ...
    guard = new VideoCallsRateLimiterGuard(
  ...
  describe('when rate limit metadata is defined', () => {
    const options: VideoCallsRateLimitOptions = {
      maxRequests: 5,
      windowSeconds: 60,
    };
    ...
    beforeEach(() => {
      (reflector.get as Mock).mockImplementation((key: string) => {
        if (key === VIDEO_CALLS_RATE_LIMIT_KEY) return options;
        return undefined;
      });
    });
    ...
    it('should allow request and set expire on first request', async () => {
      ...
      expect(redisMock.expire).toHaveBeenCalledWith(
        'video_calls:ratelimit:user-2:TestController:handler',
        60,
      );
    });
  ...
  describe('VideoCallsRateLimit decorator', () => {
    it('should set metadata on the handler', () => {
      const opts: VideoCallsRateLimitOptions = {
        maxRequests: 10,
        windowSeconds: 30,
      };
