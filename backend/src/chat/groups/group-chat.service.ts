...
export class GroupChatService {
  ...
  private async getRoom(roomId: string): Promise<GroupRoom> {
    const { data, error } = await this.supabase
      .from('chat_rooms')
      .select(
        'id,type,name,description,topic,interest_id,avatar_url,created_by,is_archived',
      )
      .eq('id', roomId)
      .maybeSingle();
  ...
  private async validateInterest(interestId?: string): Promise<void> {
    ...
    const { data, error } = await this.supabase
      ...
      .maybeSingle();
    if (error) {
      throw new Error(`Failed to validate group interest: ${error.message}`);
    }
    if (!data) throw new BadRequestException('Unknown group interest');
  }
  ...
  private async validateInvitees(
    ...
    const { data: users, error: usersError } = await this.supabase
      ...
      .in('id', memberIds);
    if (usersError) {
      throw new Error(`Failed to validate invitees: ${usersError.message}`);
    }
    ...
    if (blockError) {
      throw new Error(
        `Failed to validate block relationships: ${blockError.message}`,
      );
    }
    if ((blocks ?? []).length > 0) {
      throw new ForbiddenException(
        'A blocked user cannot be added to this group',
      );
    }
  }
  ...
  async createGroup(creatorId: string, dto: CreateGroupChatDto) {
    ...
    await Promise.all([
      this.validateInvitees(creatorId, memberIds),
      this.validateInterest(dto.interestId),
    ]);
    ...
    const { data: room, error: roomError } = await this.supabase
      ...
      .select(
        'id,type,name,description,topic,interest_id,avatar_url,created_by,is_archived,created_at',
      )
      .single();

    if (roomError || !room) {
      throw new Error(
        `Failed to create group: ${roomError?.message ?? 'no room returned'}`,
      );
    }
    ...
    if (membershipError) {
      // The room has not escaped this request yet. Compensating deletion keeps
      // mixed-version deployments safe when the membership insert fails.
      await this.supabase.from('chat_rooms').delete().eq('id', room.id);
      throw new Error(
        `Failed to add group members: ${membershipError.message}`,
      );
    }
  ...
  async updateGroup(
    ...
    await this.validateInterest(dto.interestId);
    ...
    if (dto.topic !== undefined) update.topic = dto.topic.trim() || null;
    if (dto.interestId !== undefined) update.interest_id = dto.interestId;
    if (dto.avatarUrl !== undefined) {
      update.avatar_url = dto.avatarUrl.trim() || null;
    }
    if (Object.keys(update).length === 0) {
      return this.getGroup(roomId, actorId);
    }
    ...
    if (error || !data) {
      throw new Error(
        `Failed to update group: ${error?.message ?? 'no group returned'}`,
      );
    }
  ...
  async addMembers(
    ...
    if (existingError) {
      throw new Error(
        `Failed to load group members: ${existingError.message}`,
      );
    }
    ...
    if (memberIds.length > 0) {
      ...
      await this.systemMessageService.publishToRoom(
        roomId,
        'group_members_added',
        {
          actor_id: actorId,
  ...
  async removeMember(
    ...
    if (memberId === actorId) {
      throw new BadRequestException(
        'Transfer admin ownership before leaving the group',
      );
    }
    ...
    if (membership.role === 'admin') {
      throw new BadRequestException(
        'Transfer or demote the admin before removing them',
      );
    }
  ...
  async transferAdmin(
    roomId: string,
    actorId: string,
    dto: TransferGroupAdminDto,
  ) {
    await this.requireAdmin(roomId, actorId);
    if (dto.newAdminId === actorId) {
      return { success: true, admin_id: actorId };
    }
    const target = await this.getMembership(roomId, dto.newAdminId);
    if (!target) {
      throw new BadRequestException(
        'New admin must already be a group member',
      );
    }
    ...
    if (demoteError) {
      await this.supabase
        ...
        .eq('user_id', dto.newAdminId);
      throw new Error(
        `Failed to transfer admin ownership: ${demoteError.message}`,
      );
    }
  ...
  async leaveGroup(
    ...
    if ((count ?? 0) <= 1) {
      const { error } = await this.supabase
        ...
        .eq('id', roomId);
      if (error) {
        throw new Error(`Failed to archive empty group: ${error.message}`);
      }
      await this.supabase
        .from('chat_room_members')
        .delete()
    ...
    if (membership.role === 'admin') {
      ...
      await this.transferAdmin(roomId, userId, {
        newAdminId: dto.newAdminId,
      });
    }
