...
export class ChatService {
  ...
  async forwardMessage(
    ...
    // Promise.all batch map to drastically reduce network latency during bulk forwarding.
    // Expected impact: N sequential database inserts and Centrifugo publishes become 1 concurrent block.
    const forwardTasks = Array.from(validMembershipRoomIds).map(
      async (targetRoomId) => {
        ...
        if (blocked) {
          return null; // Skip rooms where sender is blocked
        }
        ...
        if (insertResponse.error || !insertResponse.data) {
          return null; // Skip on insert failure
        }
    ...
    const results = await Promise.all(forwardTasks);
    results.forEach((res) => {
      if (res) forwardedMessages.push(res);
    });
