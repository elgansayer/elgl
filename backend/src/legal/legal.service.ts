...
const DEFAULT_TOS_SECTIONS: LegalSection[] = [
  {
    id: 'acceptance',
  ...
  },
  {
    ...
    content:
      'You retain all ownership rights to the content you submit to the service. However, by submitting content, you grant us a worldwide, non-exclusive, royalty-free licence to use, reproduce, and distribute such content on and through the service. You warrant that you have the right to grant this licence for all content you submit.',
  },
  {
    id: 'termination',
...
export class LegalService {
  ...
  private async fetchFromDatabase(
    ...
    try {
      ...
      return {
        title: row.title,
        lastUpdated: row.effective_date,
        sections: row.sections,
      };
    } catch (err) {
      this.logger.warn(
        `Failed to fetch ${type} document from Supabase, falling back to defaults`,
  ...
  private getDefaultDocument(type: 'terms' | 'privacy'): LegalDocument {
    const configKey =
      type === 'terms' ? 'TOS_EFFECTIVE_DATE' : 'PRIVACY_EFFECTIVE_DATE';
    const effectiveDate =
      this.configService.get<string>(configKey) ?? '2026-07-01';
    const sections =
      type === 'terms'
    ...
    return { title, lastUpdated: effectiveDate, sections };
  }

  async getTermsOfService(): Promise<LegalDocument> {
    const dbDoc = await this.fetchFromDatabase('terms');
    return dbDoc ?? this.getDefaultDocument('terms');
  }

  async getPrivacyPolicy(): Promise<LegalDocument> {
    const dbDoc = await this.fetchFromDatabase('privacy');
    return dbDoc ?? this.getDefaultDocument('privacy');
  }
}