...
describe('PrivacyService', () => {
  ...
  describe('archive retention', () => {
    it('caps cleanup concurrency and continues with later batches', async () => {
      tableRows.set(
        'archive_requests',
      ...
      const purge = service.purgeExpiredArchives(100);

      await vi.waitFor(() => expect(mockRemove).toHaveBeenCalledTimes(10));
      expect(maxActiveRemovals).toBe(10);
      releases.splice(0, 10).forEach((release) => release());

      await vi.waitFor(() => expect(mockRemove).toHaveBeenCalledTimes(11));
      releases.splice(0).forEach((release) => release());

      await expect(purge).resolves.toBe(11);
      expect(maxActiveRemovals).toBe(10);
    });

    it('isolates object-removal failures and counts successful purges', async () => {
      tableRows.set('archive_requests', [
        {
          id: 'failed-archive',
          user_id: 'user-1',
          status: 'ready',
          object_key: 'failed.json',
          expires_at: '2026-08-01T00:00:00.000Z',
          created_at: '2026-07-25T00:00:00.000Z',
        },
        {
