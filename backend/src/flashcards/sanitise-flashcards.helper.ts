...
/**
 * Strict DOMPurify instance for the Spaced Repetition (SRS) flashcard module.
 *
 * ALLOWED_TAGS and ALLOWED_ATTR are empty so that only plain-text content
 * survives sanitisation. Flashcard data (word tokens, original context,
 * translations, definitions, pronunciation URLs) may contain user-authored
 * content and must never include rich HTML. Stripping everything ensures
...
/**
 * Deeply sanitise a value using the strict flashcards DOMPurify config.
 *
 * Rules:
 ...
 */
export function sanitiseFlashcardsData<T>(value: T): T {
  if (typeof value === 'string') {
    return strictPurify.sanitize(value) as unknown as T;
  }

  if (Array.isArray(value)) {
    return value.map((item) => sanitiseFlashcardsData(item)) as unknown as T;
  }
  ...
  if (value !== null && typeof value === 'object') {
    ...
    for (const [key, val] of Object.entries(
      ...
      sanitised[key] = sanitiseFlashcardsData(val);
    }
