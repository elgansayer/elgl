...
/**
 * Strict DOMPurify instance for the LingQ Reading Engine.
 *
 * ALLOWED_TAGS and ALLOWED_ATTR are empty so that only plain-text content
 * survives sanitisation. Reading resource data (title, content, source_url,
 * topic, language, difficulty) is user-authored and must never contain
...
/**
 * Deeply sanitise a value using the strict reading-engine DOMPurify config.
 *
 * Rules:
 ...
 */
export function sanitiseReadingEngineData<T>(value: T): T {
  if (typeof value === 'string') {
    return strictPurify.sanitize(value) as unknown as T;
  }

  if (Array.isArray(value)) {
    return value.map((item) => sanitiseReadingEngineData(item)) as unknown as T;
  }
  ...
  if (value !== null && typeof value === 'object') {
    ...
    for (const [key, val] of Object.entries(
      ...
      sanitised[key] = sanitiseReadingEngineData(val);
    }
