...
describe('DiscoveryRateLimiterGuard', () => {
  let guard: DiscoveryRateLimiterGuard;
  let redisMock: {
    incr: Mock;
    expire: Mock;
    ttl: Mock;
  ...
  describe('when rate limit metadata is defined', () => {
    const options: DiscoveryRateLimitOptions = {
      freeMaxRequests: 30,
      vipMaxRequests: 120,
      windowSeconds: 60,
    };
    ...
    beforeEach(() => {
      (reflector.get as Mock).mockImplementation((key: string) => {
        if (key === DISCOVERY_RATE_LIMIT_KEY) return options;
        return undefined;
      });
    });
    ...
    it('should set TTL on first request in the window', async () => {
      ...
      expect(redisMock.expire).toHaveBeenCalledWith(
        'discovery:ratelimit:user-2:TestController:handler',
        60,
      );
    });
    ...
    it('should resolve VIP status from Redis cache when cached', async () => {
      ...
      const result = await guard.canActivate(context);
      expect(result).toBe(true);
      // Should not query DB since cache hit
  ...
  describe('DiscoveryRateLimit decorator', () => {
    it('should set metadata on the handler', () => {
      const opts: DiscoveryRateLimitOptions = {
        freeMaxRequests: 20,
        vipMaxRequests: 80,
        windowSeconds: 30,
      };
      ...
      DiscoveryRateLimit(opts)(DecoratorTest.prototype, 'testMethod', desc!);
