...
export const DiscoveryRateLimit = (options: DiscoveryRateLimitOptions) =>
  SetMetadata(DISCOVERY_RATE_LIMIT_KEY, options);

@Injectable()
export class DiscoveryRateLimiterGuard implements CanActivate {
  private static readonly VIP_CACHE_TTL_SECONDS = 60;

  constructor(
    @InjectPinoLogger(DiscoveryRateLimiterGuard.name)
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const options = this.reflector.get<DiscoveryRateLimitOptions>(
      DISCOVERY_RATE_LIMIT_KEY,
      handler,
    );
    ...
    const key = `discovery:ratelimit:${user.id}:${controller}:${handlerName}`;
    ...
    try {
      ...
      this.logger.error(
        ...
        'Redis error in discovery rate limiter; allowing request through',
      );
    }
    ...
    try {
      ...
      try {
        await redis.set(
          ...
          isVip ? '1' : '0',
          'EX',
          DiscoveryRateLimiterGuard.VIP_CACHE_TTL_SECONDS,
        );
      } catch {
