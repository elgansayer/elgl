...
describe('FavouritesController', () => {
  ...
  beforeEach(() => {
    service = {
      addFavourite: vi.fn().mockResolvedValue({ id: 'fav-1' }),
      removeFavourite: vi.fn().mockResolvedValue({ success: true }),
      getUserFavourites: vi.fn().mockResolvedValue([]),
    };
    controller = new FavouritesController(service as unknown as FavouritesService);
  });
  ...
  it('keeps the legacy user route owner-scoped', async () => {
    await expect(controller.getUserFavourites(request, 'user-1')).resolves.toEqual([]);
    expect(service.getUserFavourites).toHaveBeenCalledWith('user-1');
  });

  it('rejects attempts to read another user through the legacy route', async () => {
    await expect(controller.getUserFavourites(request, 'user-2')).rejects.toBeInstanceOf(
      ForbiddenException,
    );
