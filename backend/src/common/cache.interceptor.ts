import {
  CallHandler,
  ExecutionContext,
  Injectable,
  NestInterceptor,
} from '@nestjs/common';
import { Observable } from 'rxjs';
import { tap } from 'rxjs';
import { Response } from 'express';

/**
 * Cache-Control directives optimised for Cloudflare edge caching.
 *
 * Strategy:
 *  - Public static data (catalogs, reference lists): long-lived CDN cache.
 *  - User-specific read endpoints (flashcard lists, deck contents): private
 *    with short-lived browser cache and medium-lived CDN cache.
 *  - Mutation endpoints & dynamic NLP calls: no-store.
 *
 * Cloudflare respects the CDN-Cache-Control header for fine-grained
 * edge caching while Cache-Control governs browser behaviour.
 */
export const CACHE_PUBLIC_LONG = {
  'Cache-Control':
    'public, max-age=3600, s-maxage=86400, stale-while-revalidate=604800, stale-if-error=86400',
  'CDN-Cache-Control': 'public, max-age=86400, stale-while-revalidate=604800',
} as const;

export const CACHE_PUBLIC_SHORT = {
  'Cache-Control':
    'public, max-age=300, s-maxage=1800, stale-while-revalidate=600, stale-if-error=3600',
  'CDN-Cache-Control': 'public, max-age=1800, stale-while-revalidate=600',
} as const;

export const CACHE_PRIVATE_SHORT = {
  'Cache-Control':
    'private, max-age=300, s-maxage=1800, stale-while-revalidate=600, stale-if-error=3600',
  'CDN-Cache-Control': 'private, max-age=1800, stale-while-revalidate=600',
} as const;

export const CACHE_PRIVATE_MEDIUM = {
  'Cache-Control':
    'private, max-age=60, s-maxage=300, stale-while-revalidate=120, stale-if-error=600',
  'CDN-Cache-Control': 'private, max-age=300, stale-while-revalidate=120',
} as const;

export const CACHE_PRIVATE_NO_STORE = {
  'Cache-Control': 'private, no-store, no-cache, must-revalidate',
  'CDN-Cache-Control': 'private, no-store',
} as const;

@Injectable()
export class CacheControlInterceptor implements NestInterceptor {
  constructor(private readonly directive: Record<string, string>) {}

  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    const response = context.switchToHttp().getResponse<Response>();

    for (const [header, value] of Object.entries(this.directive)) {
      response.setHeader(header, value);
    }

    return next.handle().pipe(
      tap({
        error: () => {
          // On error, override caching headers to prevent storing broken responses
          response.setHeader('Cache-Control', 'private, no-store');
          response.setHeader('CDN-Cache-Control', 'private, no-store');
        },
      }),
    );
  }
}