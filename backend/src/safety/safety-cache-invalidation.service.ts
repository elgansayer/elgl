...
export class SafetyCacheInvalidationService {
   ...
   *    scan and delete matching keys using SCAN to prevent DoS vulnerabilities
   *    caused by the blocking KEYS command.
   */
    ...
    try {
      ...
      for (const pattern of SAFETY_AFFECTED_CACHE_PATTERNS) {
        if (pattern.endsWith(':*')) {
          ...
          // Prefix pattern – use SCAN instead of KEYS to avoid blocking event loop
