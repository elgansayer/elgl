...
describe('language challenge economy migration (#1157)', () => {
  ...
  it('creates challenge, participant, and daily activity persistence with bounded indexes', () => {
    expect(sql).toMatch(/CREATE TABLE IF NOT EXISTS public\.language_challenges/);
    expect(sql).toMatch(/CREATE TABLE IF NOT EXISTS public\.language_challenge_participants/);
    expect(sql).toMatch(/CREATE TABLE IF NOT EXISTS public\.language_challenge_daily_activity/);
  ...
  it('keeps browser writes behind the authenticated backend boundary', () => {
    expect(sql).toMatch(/ALTER TABLE public\.language_challenges ENABLE ROW LEVEL SECURITY/);
    expect(sql).toMatch(/language_challenge_participants_select_own/);
    expect(sql).toMatch(/language_challenge_activity_select_own/);
    expect(sql).toMatch(/REVOKE ALL ON FUNCTION public\.join_language_challenge\(UUID, UUID\) FROM PUBLIC, anon, authenticated/);
    expect(sql).toMatch(/GRANT EXECUTE ON FUNCTION public\.join_language_challenge\(UUID, UUID\) TO service_role/);
  });
  ...
  it('makes entry charging and prize-pool contribution one atomic RPC', () => {
    const joinFunction = sql.slice(
      ...
      sql.indexOf('CREATE OR REPLACE FUNCTION public.checkin_language_challenge'),
    );

    expect(joinFunction).toMatch(/FOR UPDATE/);
    expect(joinFunction).toMatch(/coins_balance = coins_balance - v_challenge\.entry_fee_coins/);
    expect(joinFunction).toMatch(/INSERT INTO public\.language_challenge_participants/);
    expect(joinFunction).toMatch(/prize_pool_coins = prize_pool_coins \+ v_challenge\.entry_fee_coins/);
  ...
  it('makes daily progress retry-safe and defines days in UTC', () => {
    const checkinFunction = sql.slice(
      sql.indexOf('CREATE OR REPLACE FUNCTION public.checkin_language_challenge'),
      sql.indexOf('CREATE OR REPLACE FUNCTION public.claim_language_challenge_prize'),
    );

    expect(checkinFunction).toMatch(/now\(\) AT TIME ZONE 'UTC'/);
    expect(checkinFunction).toMatch(/ON CONFLICT \(challenge_id, user_id, activity_date\) DO NOTHING/);
  ...
  it('settles all eligible winners exactly once under a challenge row lock', () => {
    ...
    expect(claimFunction).toMatch(/coins_balance = u\.coins_balance \+ v_share/);
    expect(claimFunction).toMatch(/'challenge_prize'/);
    expect(claimFunction).toMatch(/status = 'completed', completed_at = now\(\)/);
  ...
  it('is restart-safe without rewriting deployed migration history', () => {
    ...
    expect(sql).toMatch(/CREATE OR REPLACE FUNCTION public\.join_language_challenge/);
