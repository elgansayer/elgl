...
describe('per-user chat priority pins migration (#1167)', () => {
  ...
  it('creates an idempotent per-user pin relation with cascade deletion', () => {
    ...
    expect(sql).toMatch(/user_id UUID NOT NULL REFERENCES public\.users\(id\) ON DELETE CASCADE/i);
    expect(sql).toMatch(/room_id UUID NOT NULL REFERENCES public\.chat_rooms\(id\) ON DELETE CASCADE/i);
    expect(sql).toMatch(/PRIMARY KEY \(user_id, room_id\)/i);
  });

  it('indexes the bounded user-first read path', () => {
    expect(sql).toMatch(/CREATE INDEX IF NOT EXISTS idx_chat_room_pins_user_created/i);
    expect(sql).toMatch(/ON public\.chat_room_pins \(user_id, created_at, room_id\)/i);
  });

  it('enables RLS and scopes reads and mutations to the authenticated owner', () => {
    expect(sql).toMatch(/ALTER TABLE public\.chat_room_pins ENABLE ROW LEVEL SECURITY/i);
  ...
  it('backfills the legacy shared pin only as an initial per-member preference', () => {
    expect(sql).toMatch(/JOIN public\.chat_rooms room ON room\.id = member\.room_id/i);
