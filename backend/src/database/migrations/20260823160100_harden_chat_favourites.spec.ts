...
describe('harden chat favourites migration', () => {
  ...
  it('canonicalises the payload from chat_messages instead of trusting a client snapshot', () => {
    expect(migration).toContain('CREATE OR REPLACE FUNCTION public.normalise_chat_favourite()');
  ...
  it('enforces room membership even when the backend uses service-role database access', () => {
    ...
    expect(migration).toContain('Cannot favourite a message outside your rooms');
  });

  it('bounds notes, strips reusable view-once media URLs, and makes retries idempotent', () => {
    expect(migration).toMatch(/CHAR_LENGTH\(COALESCE\(NEW\.note_text, ''\)\) > 500/);
    expect(migration).toContain("canonical_payload := canonical_payload - 'media_url'");
    expect(migration).toContain('pg_advisory_xact_lock');
    expect(migration).toMatch(/WHERE existing\.user_id = NEW\.user_id[\s\S]*existing\.message_id = canonical_message_id/);
  ...
  it('keeps the migration additive and replay-safe', () => {
    ...
    expect(migration).toContain('DROP TRIGGER IF EXISTS normalise_chat_favourite_before_write');
    expect(migration).toContain('CREATE TRIGGER normalise_chat_favourite_before_write');
  });
});
