...
export class StreakResetCron {
  ...
  async resetInactiveStreaks(): Promise<void> {
    ...
    // We use created_at as a proxy for last activity since we don't have a dedicated last_active_at column
    // In production, you'd use a last_active_at column on the users table
    ...
    this.logger.log(`Found ${usersToReset.length} users to reset streaks for.`);
