...
test.describe('HelloTalk RTL & Monetary Invariant Verification', () => {
  test('RTL locales automatically mirror layout and maintain logical CSS boundaries', async ({ page }) => {
    await page.goto('/');
    ...
    const hasDualCurrencyConsumer = pageContent.includes('8 UKP / $10 USD') || pageContent.includes('8 UKP');
    const hasDualCurrencyDeveloper = pageContent.includes('20 UKP / $26 USD') || pageContent.includes('20 UKP');
  ...
  test('Developer Sandbox executes PostGIS query and renders distance_metres correctly', async ({ page }) => {
    await page.goto('/');
    ...
    if (await postgisTab.isVisible()) {
      ...
      const resultsContainer = page.locator('section', { hasText: 'PostGIS Geography Query Simulator' });
