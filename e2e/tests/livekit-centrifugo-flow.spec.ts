...
test.describe('Real-Time Engine Verification (Centrifugo v5 & LiveKit SFU)', () => {
  test('Centrifugo connection token and LiveKit stage simulation update signals reactively', async ({ page }) => {
    await page.goto('/');
    ...
    if (await centrifugoTab.isVisible()) {
      ...
      if (await triggerFanout.isVisible()) {
        ...
        const logContainer = page.locator('section', { hasText: 'Live Architectural Diagnostics Stream' });
