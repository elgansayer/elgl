/**
 * Artillery helper functions for SRS (Spaced Repetition) Flashcards load testing.
 *
 * Provides custom utility hooks and data generators for the
 * srs-flashcards.load.yml test suite.
 */
...
function beforeRequest(requestParams, context, events, next) {
  if (requestParams.headers) {
    requestParams.headers['X-Load-Test-Id'] =
      context.vars.loadTestId || `srs-load-${Date.now()}`;
  }
  return next();
}
