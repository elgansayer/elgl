...
- [ ] Install `unzip` (required by fnm install script) using `sudo apt-get update && sudo apt-get install -y unzip`. After installation, verify with `unzip --version`. Then retry the fnm installation steps above.
