- [ ] Fix QA test failure: `[WebServer]   error: TypeError: fetch failed`. Per `qa_errors.log`, this is the surface symptom of an `AggregateError [ECONNREFUSED]` on every Angular `HttpClient` call made during the e2e `webServer` boot (e.g. `GET http://localhost:3000/api/economy/catalog`, `/api/safety/blocked-ids`, `/api/economy/daily-check-in`, `/api/chat/token`, all logged as `Http failure response ... 0 undefined`). Root cause: the NestJS backend (`http://localhost:3000`) is not up/reachable when the Angular dev server (started by `e2e/playwright.config.ts`'s `webServer`) starts making these calls, so every request is refused at the TCP level. Investigate why the `api` service isn't listening on port 3000 during the QA run, e.g. `qa-loop.sh` only starting the frontend `webServer` and never starting/waiting on the backend, backend crashing on boot (check for a startup error, possibly related to the in-flight `economy`/`monetisation` changes visible in `git status`), or a port/env mismatch between backend config and the frontend's API base URL. Fix so the backend is reliably running and reachable on `localhost:3000` before/while the e2e suite runs, then rerun the QA suite to confirm the `fetch failed` errors are gone.
