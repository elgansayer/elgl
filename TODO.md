...
- [ ] Ensure Node.js and npm are accessible in PATH after installation (e.g., run `export PATH=$PATH:/usr/local/bin` or verify with `node --version` and `npm --version`). (The previous direct binary download may have succeeded but the binaries are not in the PATH.)
