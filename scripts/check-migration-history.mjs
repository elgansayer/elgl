...
const allowedLegacyDuplicate = new Map([
  [
    '013',
    new Set(['013_chat_message_enhancements.sql', '013_multiple_native_languages.sql']),
  ],
]);
...
for (const file of files) {
  ...
  if (!match) {
    errors.push(`${file}: expected NNN_name.sql or YYYYMMDDHHMMSS_name.sql`);
    continue;
  }
...
for (const [id, rows] of [...byId.entries()].sort(([a], [b]) => a.localeCompare(b))) {
...
const legacyNumericIds = [...byId.keys()]
  .filter((id) => id.length === 3)
  .map(Number)
  .sort((a, b) => a - b);

for (let i = 1; i < legacyNumericIds.length; i += 1) {
  if (legacyNumericIds[i] < legacyNumericIds[i - 1]) {
    errors.push('legacy migration IDs are not monotonically ordered');
    break;
  }
}
