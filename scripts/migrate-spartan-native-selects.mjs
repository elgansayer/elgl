...
function migrateOpeningTag(tag) {
  let next = tag.replace(/^<select\b/, '<hlm-native-select');

  next = next.replace(/\[id\]\s*=\s*("[^"]*"|'[^']*')/g, '[selectId]=$1');
  next = next.replace(/(?<![\w-])id\s*=\s*("[^"]*"|'[^']*')/g, 'selectId=$1');
  ...
  next = next.replace(/\[class\]\s*=\s*("[^"]*"|'[^']*')/g, (match, expression) => {
    return `${match} [selectClass]=${expression}`;
  });
  next = next.replace(/(?<![\w-])class\s*=\s*("([^"]*)"|'([^']*)')/g, (match, quoted, doubleValue, singleValue) => {
    const value = doubleValue ?? singleValue ?? '';
    const quote = quoted[0];
    return `${match} selectClass=${quote}${value}${quote}`;
  });
...
function transformTemplate(source, ownerPath) {
  let changed = false;
  let next = source.replace(/<select\b[\s\S]*?>/g, (tag) => {
    changed = true;
    return migrateOpeningTag(tag);
  });
  next = next.replace(/<\/select>/g, () => {
    changed = true;
    return '</hlm-native-select>';
  });
...
function ensureNamedImport(source) {
  const moduleName = '@spartan-ng/helm/native-select';
  const moduleImport = /import\s*\{([^}]*)\}\s*from\s*['"]@spartan-ng\/helm\/native-select['"];?/;
  const match = moduleImport.exec(source);
  if (match) {
    const names = match[1].split(',').map((name) => name.trim()).filter(Boolean);
    if (names.some((name) => name.split(/\s+as\s+/)[0] === 'HlmNativeSelect')) return source;
    const replacement = match[0].replace('{', '{ HlmNativeSelect,');
    return source.slice(0, match.index) + replacement + source.slice(match.index + match[0].length);
  }
  return `import { HlmNativeSelect } from '${moduleName}';\n${source}`;
}
...
function ensureComponentImport(source) {
  ...
  if (match) {
    ...
    if (close !== -1 && /\bHlmNativeSelect\b/.test(source.slice(start, close))) return source;
    return source.slice(0, start) + 'HlmNativeSelect, ' + source.slice(start);
  }
...
console.log(`Migrated native selects to owned Spartan Native Select across ${required.size} Angular component source file(s).`);
