...
awk '
  function is_canonical_path(key) {
    return key ~ /^(FACTORY_REPOSITORY|FACTORY_BASE_BRANCH|FACTORY_STATE_DIR|FACTORY_LOG_DIR|FACTORY_PROFILE_STORE|FACTORY_WORKTREE_DIR|FACTORY_RECOVERY_DIR|FACTORY_PODMAN_PATH|FACTORY_TASK_IMAGE|FACTORY_MINIMUM_FREE_DISK_GIB|FACTORY_MAX_PARALLEL_JOBS|FACTORY_REQUIRE_TRUSTED_INTAKE|FACTORY_TRUSTED_GITHUB_ACTORS|FACTORY_REQUIRE_READY_LABEL|FACTORY_READY_LABEL|GEMINI_ENABLED)$/
  }
  {
    key = $0
    sub(/^[[:space:]]*/, "", key)
