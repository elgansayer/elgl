...
awk '
  function is_canonical_path(key) {
    return key ~ /^(FACTORY_REPOSITORY|FACTORY_STATE_DIR|FACTORY_LOG_DIR|FACTORY_PROFILE_STORE|FACTORY_WORKTREE_DIR|FACTORY_PODMAN_PATH|FACTORY_TASK_IMAGE|FACTORY_MINIMUM_FREE_DISK_GIB)$/
  }
  {
    key = $0
    sub(/^[[:space:]]*/, "", key)
...
install -d -o hellotalk-factory -g hellotalk-factory -m 0750 "$STATE_DIR" "$STATE_DIR/home" "$STATE_DIR/profiles" "$STATE_DIR/worktrees" "$LOG_DIR"
chown -R hellotalk-factory:hellotalk-factory "$STATE_DIR/repository" "$STATE_DIR/profiles" "$STATE_DIR/worktrees" "$STATE_DIR/home"
