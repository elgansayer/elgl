...
## 2026-09-12 - [Replace blocking Redis KEYS with SCAN]
**Learning:** Using KEYS in Redis is a blocking operation that degrades performance and causes potential DoS vulnerabilities on large datasets. It blocks the Redis event loop when scanning for keys.
**Action:** Use a non-blocking SCAN command via a cursor loop when iterating and deleting cached keys based on patterns (e.g., cache invalidation).
