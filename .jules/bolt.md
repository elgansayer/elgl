...
## 2026-09-08 - [Batch Supabase Storage and DB calls with Promise.allSettled]
**Learning:** Sequential awaits when updating rows or removing objects from Supabase storage inside a loop cause significant, additive delay to the node event loop when processing large sets of records (like GDPR archive purge requests).
**Action:** Replace single sequential Supabase database and storage calls inside a `for...of` loop with concurrent `Promise.allSettled` batches using `.map` to improve backend network latency.
