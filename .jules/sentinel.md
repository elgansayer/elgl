...
## 2026-08-31 - [Replace Insecure Math.random() with randomInt()]
**Vulnerability:** Weak random number generation (`Math.random()`) used for security-sensitive logic, economy rewards, or tokens in the backend.
**Learning:** `Math.random()` generates predictable values. Using it to generate economy rewards or identifiers for security contexts makes the application susceptible to manipulation.
**Prevention:** Always use Node.js's native `crypto.randomInt()` or `crypto.randomBytes()` for generating unique identifiers, tokens, security keys, or economy rewards in backend environments.
