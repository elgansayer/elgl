...
## 2026-09-02 - [Replace Insecure Math.random() in Economy and Dashboard Services]
**Vulnerability:** Weak random number generation (`Math.random()`) used for economic rewards generation in `economy.service.ts` and `host-dashboard.service.ts`.
**Learning:** `Math.random()` is cryptographically weak and predictable, creating vulnerabilities where economy mechanics could be predicted or exploited.
**Prevention:** Always use Node.js's native `crypto.randomInt()` or `crypto.randomBytes()` for anything involving rewards or sensitive generation.
