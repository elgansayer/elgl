...
## 2026-09-02 - [Replace Predictable Math.random() in Economy Service]
**Vulnerability:** Weak random number generation (`Math.random()`) was used to generate economy rewards (coins).
**Learning:** `Math.random()` generates predictable values. Using it to generate rewards or tokens makes the application susceptible to abuse and predictability.
**Prevention:** Always use Node.js's native `crypto.randomInt()` or `crypto.randomBytes()` for generating economy rewards, tokens, or security keys.
