...
## 2026-08-31 - [Replace Insecure Math.random() with crypto.randomInt()]

**Vulnerability:** Weak random number generation (`Math.random()`) used for security-sensitive logic and economy rewards in `economy.service.ts` and `host-dashboard.service.ts`.
**Learning:** `Math.random()` generates predictable values. Using it to generate rewards or tokens can negatively impact economy stability or allow exploits.
**Prevention:** Always use Node.js's native `crypto.randomInt()` or `crypto.randomBytes()` for generating security-sensitive random values or economy rewards.
