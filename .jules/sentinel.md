...
## 2026-09-11 - [Strict Keys Command Prohibition in Cache Invalidation]
**Vulnerability:** The `AdminService` used the blocking `KEYS` command to invalidate caches (e.g., `redis.keys('admin:users:list:*')`). This stalls the Redis event loop, creating a Denial of Service (DoS) vector on production-scale instances.
**Learning:** Never use `KEYS` in production code. It runs in O(N) time across the entire key space and prevents any other commands from executing until it completes.
**Prevention:** Always use the non-blocking `SCAN` command with an iterative cursor loop for pattern matching in cache invalidations.
