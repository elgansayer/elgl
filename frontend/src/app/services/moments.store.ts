...
export class MomentsStore {
  ...
  async toggleLike(momentId: string): Promise<void> {
    try {
      ...
      this.feed.update(list => list.map(m => {
        if (m.id === momentId) {
  ...
  async loadComments(momentId: string): Promise<MomentComment[]> {
    try {
      const list = await firstValueFrom(
        this.http.get<MomentComment[]>(`${this.baseUrl}/${momentId}/comments`, { headers: this.getHeaders() })
      );
      this.feed.update(moments => moments.map(m => m.id === momentId ? { ...m, comments: list } : m));
      return list;
    } catch (e) {
  ...
  async addComment(momentId: string, payload: {
    text_content?: string;
    correction_payload?: {
      original: string;
      corrected: string;
  ...
  async togglePin(momentId: string): Promise<void> {
    try {
      const updated = await firstValueFrom(
        this.http.patch<MomentRecord>(`${this.baseUrl}/${momentId}/pin`, {}, { headers: this.getHeaders() })
      );
      this.feed.update(list => {
        const next = list.map(m => m.id === momentId ? { ...m, is_pinned: updated.is_pinned } : m);
