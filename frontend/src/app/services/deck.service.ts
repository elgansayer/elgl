...
export class DeckService {
  ...
  async getDecks(): Promise<Deck[]> {
    return firstValueFrom(this.http.get<Deck[]>(this.baseUrl, { headers: this.getHeaders() }));
  }
  ...
  async createDeck(dto: CreateDeckDto): Promise<Deck> {
    return firstValueFrom(this.http.post<Deck>(this.baseUrl, dto, { headers: this.getHeaders() }));
  }
  ...
  async removeFlashcardFromDeck(deckId: string, flashcardId: string): Promise<void> {
    return firstValueFrom(
      this.http.delete<void>(`${this.baseUrl}/${deckId}/flashcards/${flashcardId}`, {
        headers: this.getHeaders(),
      }),
    );
  }
  ...
  async getDeckFlashcards(deckId: string): Promise<string[]> {
    const response = await firstValueFrom(
      this.http.get<{ id: string }[]>(`${this.baseUrl}/${deckId}/flashcards`, {
        headers: this.getHeaders(),
      }),
    );
