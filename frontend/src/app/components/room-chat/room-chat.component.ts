...
@Component({
  ...
  template: `
    <div
      class="bg-white dark:bg-slate-800 rounded-3xl shadow-xl border border-slate-200 dark:border-slate-700 flex flex-col h-96 overflow-hidden"
    >
      <!-- Header -->
      <div
        class="bg-slate-50 dark:bg-slate-900/60 px-4 py-3 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between"
      >
        <span
          class="font-black text-xs text-slate-800 dark:text-slate-200 flex items-center gap-1.5"
        >
      ...
      <div class="flex-1 p-4 overflow-y-auto space-y-2 text-xs">
        @if (activeTab() === 'chat') {
          ...
          @for (msg of store.roomMessages(); track msg) {
            <div class="p-2 rounded-xl bg-slate-50 dark:bg-slate-700/50">
              <div
                class="flex items-center justify-between font-bold text-[11px] text-slate-600 dark:text-slate-300 mb-0.5"
              >
        ...
        @if (activeTab() === 'subtitles') {
          ...
          @for (cap of store.captions(); track cap) {
            <div
              class="p-2.5 rounded-xl bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-800"
            >
              <span class="font-bold text-[10px] text-purple-900 dark:text-purple-300 block mb-1"
      ...
      @if (activeTab() === 'chat') {
        <div
          class="p-3 bg-slate-50 dark:bg-slate-900/60 border-t border-slate-200 dark:border-slate-700 flex gap-2"
        >
          <input
            type="text"
            [(ngModel)]="inputText"
      ...
      @if (activeTab() === 'subtitles' && store.isSpeaker()) {
        <div
          class="p-3 bg-purple-50 dark:bg-purple-950/60 border-t border-purple-200 dark:border-purple-800 flex gap-2"
        >
          <input
            type="text"
