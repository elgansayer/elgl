  ...
  template: `
    <div class="fixed inset-0 z-50" (click)="close()" (touchstart)="close()">
      <div class="absolute" [style.top.px]="y()" [style.left.px]="x()">
        <div class="bg-surface-200 border border-gray-600 rounded-lg shadow-xl py-1 min-w-[160px]" (click)="$event.stopPropagation()" (touchstart)="$event.stopPropagation()">
          <button (click)="onCopy()" class="w-full text-left px-4 py-2 text-sm text-gray-200 hover:bg-surface-100 flex items-center gap-2">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
            Copy
          </button>
          <button (click)="onFavourite()" class="w-full text-left px-4 py-2 text-sm text-gray-200 hover:bg-surface-100 flex items-center gap-2">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
            Favourite
          </button>
          <button (click)="onReport()" class="w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-surface-100 flex items-center gap-2">
