  ...
  template: `
    <div class="max-w-6xl mx-auto p-4">
      ...
      <span
        ...
        stepPosition="bottom"
      >
        <a routerLink="/cart" class="mb-6 block text-sm font-medium text-indigo-400 underline">{{
          'cart.title' | t
        }}</a>
      </span>
      @if (message()) {
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4" role="list">
        @for (item of items(); track item.id) {
          <div class="rounded-xl bg-surface p-3 shadow flex flex-col" role="listitem">
            <div
              class="h-24 sm:h-28 w-full rounded-lg bg-neutral-700 mb-2 flex items-center justify-center text-3xl sm:text-4xl"
              aria-hidden="true"
            >
              {{ item.imageUrl ? '' : '🎁' }}
            </div>
            ...
            <p
              class="mt-auto pt-1 font-semibold text-indigo-400 text-sm sm:text-base"
