...
@Component({
  ...
  template: `
    <main class="max-w-6xl mx-auto p-4" aria-labelledby="shop-title">
      <h1 id="shop-title" class="text-xl font-bold mb-4">{{ 'shop.title' | t }}</h1>
      ...
      <span joyrideStep="economyTour@shopLink" [text]="'tour.shopLinkDesc' | t" stepPosition="bottom">
        <a routerLink="/cart" class="mb-6 block text-sm font-medium text-indigo-400 underline" [attr.aria-label]="'shop.viewCart' | t">{{ 'cart.title' | t }}</a>
      </span>
      @if (message()) {
        <p class="mb-4 text-sm text-indigo-300" role="status" aria-live="polite">{{ message() }}</p>
      }
      <div class="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4" role="list">
        @for (item of items(); track item.id) {
          <div class="rounded-xl bg-surface p-3 shadow flex flex-col" role="listitem">
            ...
            <p class="mt-auto pt-1 font-semibold text-indigo-400 text-sm sm:text-base" [attr.aria-label]="'shop.itemPrice' | t: { price: item.price }">
            ...
            <button
              ...
              [attr.aria-label]="'cart.addItem' | t: { name: item.name }">
              {{ 'cart.add' | t }}
            </button>
          </div>
        }
