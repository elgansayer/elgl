      ...
      <div class="flex flex-col h-full w-full bg-slate-900 p-4 rounded-2xl">
        ...
        <div class="flex-1 grid gap-4 transition-all duration-300" [class]="gridClass()">
          ...
          <div class="relative bg-black rounded-xl overflow-hidden border-2 border-slate-700 flex items-center justify-center shadow-lg">
            @if (!hostVideoTrack()) {
              <p class="text-slate-500 text-sm px-4 text-center">{{ 'videoRoom.waitingForHost' | t }}</p>
            }
          ...
          @if (hasCoHost()) {
            <div class="relative bg-black rounded-xl overflow-hidden border-2 border-blue-500 flex items-center justify-center shadow-lg animate-fade-in">
              @if (!coHostVideoTrack()) {
                <p class="text-slate-500 text-sm px-4 text-center">{{ 'videoRoom.waitingForCoHost' | t }}</p>
              }
...
export class VideoRoomComponent {
  ...
  readonly isHost = computed(() => this.authService.currentUser()?.id === this.store.currentRoom()?.host_id);
