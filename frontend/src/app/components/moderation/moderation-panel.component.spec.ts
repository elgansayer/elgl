  ...
  describe('RTL logical CSS compliance', () => {
    ...
    beforeAll(() => {
      templateContent = readFileSync(
        resolve(__dirname, 'moderation-panel.html'),
        'utf-8',
      );
    });
    ...
    it('should use i18n translate pipe for user-facing strings', () => {
      const keys = [
        ...
        'moderation.analyse', 'moderation.reported_user', 'moderation.reason',
        'moderation.date', 'moderation.itemsListAria', 'moderation.reportItemAria',
        'moderation.riskAnalysisAria', 'moderation.riskScore', 'moderation.flags',
        'moderation.noFlags',
        'safety.moderation.loadingTitle', 'safety.moderation.loadError',
    ...
    it('should not hardcode English user-facing strings', () => {
      ...
      expect(templateContent).not.toMatch(/Risk Analysis/);
      expect(templateContent).not.toMatch(/No flags/);
    });
  });
});
