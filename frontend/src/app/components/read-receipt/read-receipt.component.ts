...
@Component({
  ...
  template: `
    ...
    >
      ...
      @if (showDetail()) {
        <div
          class="absolute bottom-full end-0 mb-1 bg-gray-900 border border-gray-700 rounded-lg p-2 shadow-xl z-50 min-w-[180px]"
          role="tooltip"
        >
          <p class="text-xs text-gray-300 font-medium mb-1">
            {{ tooltipTitle() }}
          </p>
            ...
            @for (reader of displayedReaders(); track reader.userId) {
              <div class="flex items-center gap-2 py-1">
                @if (reader.avatarUrl) {
                  ...
                  <div
                    class="w-5 h-5 rounded-full bg-gray-600 flex items-center justify-center text-xs text-white"
                  >
                    {{ reader.displayName.charAt(0) }}
                  </div>
                }
                <span class="text-xs text-gray-200 truncate flex-1">
                  {{ reader.displayName }}
                </span>
