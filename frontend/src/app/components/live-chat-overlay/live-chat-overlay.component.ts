import { Component, ElementRef, effect, inject, input, signal, viewChild, OnDestroy, OnInit } from '@angular/core';
...
@Component({
  ...
  template: `
    ...
    <div class="absolute bottom-0 start-0 w-full h-72 p-4 flex flex-col justify-end pointer-events-none bg-gradient-to-t from-black/80 via-black/30 to-transparent z-50">
      
      <!-- Scrollable message list with top-fade mask -->
      <div #scrollContainer class="overflow-y-auto flex flex-col gap-3 max-h-full pointer-events-auto scrollbar-hide mask-image-fade-top pb-2">
        @for (msg of messages(); track msg.id) {
          <div class="flex flex-col bg-black/40 rounded-xl p-2.5 max-w-[85%] backdrop-blur-md animate-fade-in border border-white/10 shadow-sm">
