...
@Component({
  ...
  template: `
    @if (isOpen()) {
      ...
      <!-- Drawer Panel -->
      <div
        class="fixed top-0 bottom-0 end-0 z-50 w-80 max-w-full bg-[#121212] shadow-2xl flex flex-col border-s border-slate-800 transition-transform duration-300 ease-in-out"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div class="flex items-center justify-between p-4 border-b border-slate-800">
          ...
          <button
            (click)="close()"
            class="p-2 text-slate-400 hover:text-slate-200 rounded-full hover:bg-slate-800 transition-colors focus:outline-none focus:ring-2 focus:ring-primary"
            ...
            >
              ...
              <div class="flex-1 min-w-0 text-start">
                <div class="flex items-center gap-2">
                  ...
                  @if (user.is_vip) {
                    <span
                      class="px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-yellow-900 bg-yellow-400 rounded-sm shrink-0"
                    >
