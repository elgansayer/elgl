 ...
 *
 * Verifies that all discovery-related components and services exclusively
...
describe('RTL Logical CSS Properties - Discovery Map', () => {
  ...
  it('should use logical direction utilities (ps-/pe-/ms-/me-/border-s/border-e) in discovery templates', () => {
    const logicalMatches = allContent.match(LOGICAL_CLASS_REGEX) ?? [];
    expect(logicalMatches.length).toBeGreaterThan(0);
  });

  it('should not contain physical direction Tailwind classes (pl-/pr-/ml-/mr-/left-/right-/border-l/border-r) in discovery templates', () => {
