...
@Component({
  ...
  template: `
    <div class="bg-surface p-4 rounded-xl shadow-lg">
      ...
      <input [(ngModel)]="question" class="w-full p-2 rounded bg-surface-2 border-s border-surface-4 mb-2" placeholder="{{ 'quickPoll.questionPlaceholder' | t }}" />
      @for (opt of options(); track $index) {
        <div class="flex items-center gap-2 mb-1">
          <input [(ngModel)]="options()[$index]" class="flex-1 p-2 rounded bg-surface-2 border-s border-surface-4" [placeholder]="'quickPoll.optionPlaceholder' | t : { n: $index + 1 }" />
          @if (options().length > 2) {
            <button (click)="removeOption($index)" class="text-red-400 text-sm">{{ 'quickPoll.removeOption' | t }}</button>
          }
        </div>
      }
