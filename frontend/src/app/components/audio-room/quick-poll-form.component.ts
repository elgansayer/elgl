    ...
    <div class="bg-surface-300 p-4 rounded-xl shadow-lg">
      ...
      @for (opt of options(); track $index) {
        <div class="flex items-center gap-2 mb-1">
          ...
          />
          @if (options().length > 2) {
            <button (click)="removeOption($index)" class="text-danger text-sm">
              {{ 'quickPoll.removeOption' | t }}
            </button>
          }
        </div>
      }
      <button (click)="addOption()" class="text-primary text-sm mb-3">
      ...
      <div class="flex justify-end gap-2">
        ...
        <button (click)="submit()" class="px-4 py-2 rounded bg-primary text-on-fill">
