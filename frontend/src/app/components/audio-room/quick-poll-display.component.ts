...
@Component({
  ...
  template: `
    <div class="bg-surface p-4 rounded-xl shadow-lg">
      ...
      @for (opt of data().options; track $index; let idx = $index) {
        <div class="flex items-center gap-2 mb-2">
          <div class="flex-1 bg-surface-2 rounded-full h-6 relative overflow-hidden">
            ...
            <div class="absolute inset-y-0 start-0 bg-accent rounded-full transition-all" [style.width.%]="widthPct"></div>
      ...
      <p class="text-xs text-surface-5 mt-2">{{ 'quickPoll.totalVotes' | t : { count: data().totalVotes } }}</p>
    </div>
  `,
})
