  ...
  template: `
    <div class="fixed inset-0 z-50 bg-black flex flex-col">
      ...
      <div class="flex-1 relative bg-gray-900">
        ...
        <div class="absolute top-4 end-4 w-32 h-48 rounded-xl overflow-hidden shadow-lg border-2 border-white/30">
          @if (localVideoTrack()) {
            <video
              #localVideo
              autoplay
      ...
      <div class="bg-gray-900/95 backdrop-blur-sm px-6 py-4 flex items-center justify-center gap-4">
        ...
        <app-button-secondary
          [customClass]="isAudioMuted() ? 'bg-red-500 hover:bg-red-600 rounded-full w-14 h-14' : 'bg-white/20 hover:bg-white/30 rounded-full w-14 h-14'"
          (clicked)="toggleAudio()"
        >
          @if (isAudioMuted()) {
        ...
        <!-- Mute/Unmute Video -->
        <app-button-secondary
          [customClass]="isVideoMuted() ? 'bg-red-500 hover:bg-red-600 rounded-full w-14 h-14' : 'bg-white/20 hover:bg-white/30 rounded-full w-14 h-14'"
          (clicked)="toggleVideo()"
        >
          @if (isVideoMuted()) {
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export class VideoCallComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    try {
      ...
      const token = await this.livekitService.getToken(
        this.roomName(),
        this.otherUserId()
      );
      ...
      await this.room.connect(
        this.livekitService.getLiveKitUrl(),
        token
      );
