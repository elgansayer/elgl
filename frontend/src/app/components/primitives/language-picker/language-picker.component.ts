...
export function getLanguageFlag(code: string): string {
  const flags: Record<string, string> = {
    ...
    ko: '🇰🇷', zh: '🇨🇳', ar: '🇸🇦', ru: '🇷🇺', hi: '🇮🇳', tr: '🇹🇷', no: '🇳🇴',
    nl: '🇳🇱', el: '🇬🇷', uk: '🇺🇦', ur: '🇵🇰', sw: '🇰🇪', vi: '🇻🇳', pl: '🇵🇱',
    he: '🇮🇱', fa: '🇮🇷',
    sv: '🇸🇪', fi: '🇫🇮', da: '🇩🇰', id: '🇮🇩', th: '🇹🇭', ms: '🇲🇾',
    cs: '🇨🇿', hu: '🇭🇺', ro: '🇷🇴', bg: '🇧🇬', hr: '🇭🇷', sk: '🇸🇰',
    sr: '🇷🇸', sl: '🇸🇮', sq: '🇦🇱', mk: '🇲🇰', bs: '🇧🇦',
...
export class LanguagePickerComponent {
  ...
  readonly allLanguages = computed(() => {
    try {
      ...
      return ALL_LANGUAGE_CODES.map(code => ({ code, name: code, nativeName: code, flag: getLanguageFlag(code) }));
    }
  });
  ...
  readonly selectedLanguage = computed(() => {
    ...
    if (!code) return null;
    return this.allLanguages().find(l => l.code === code) || null;
  });
