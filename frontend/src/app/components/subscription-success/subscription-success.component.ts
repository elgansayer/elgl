  ...
  template: `
    ...
    >
      <div class="text-center max-w-md w-full">
        ...
        <h1 class="text-2xl sm:text-3xl font-bold text-white mb-4">{{ 'subscription.success.title' | t }}</h1>
        <p class="text-slate-300 mb-8 text-sm sm:text-base">{{ 'subscription.success.message' | t }}</p>
        <button
          (click)="goToDashboard()"
