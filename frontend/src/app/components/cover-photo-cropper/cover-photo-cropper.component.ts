    ...
    <hlm-dialog [state]="dialogState()" (stateChanged)="onDialogStateChanged($event)">
      <hlm-dialog-content
        *hlmDialogPortal
        ...
        class="max-h-[calc(100dvh-1.5rem)] w-[calc(100vw-1.5rem)] max-w-lg gap-5 overflow-y-auto !rounded-sheet border border-surface-100 bg-surface-200 p-4 shadow-lift sm:w-full sm:max-w-lg sm:p-6"
      >
        <h3
          hlmDialogTitle
