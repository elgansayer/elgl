...
- **Custom-scheme deep links:** `hellotalk://groups/<id>`, `hellotalk://events/<id>`, and `hellotalk://audio-rooms/<id>` emit `/groups/:id`, `/events/:id`, and `/audio-rooms/:id`. None of those parameterized Angular routes is registered, so each link falls through to the wildcard redirect.
