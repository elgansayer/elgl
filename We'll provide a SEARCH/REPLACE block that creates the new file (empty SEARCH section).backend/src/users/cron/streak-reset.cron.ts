...
export class StreakResetCron {
  ...
  async resetInactiveStreaks(): Promise<void> {
    ...
    // In production, you'd use a last_active_at timestamp column
    ...
    if (!usersToReset || usersToReset.length === 0) {
      this.logger.log('No users found with expired streaks to reset.');
      return;
    }

    this.logger.log(`Found ${usersToReset.length} users with expired streaks. Resetting...`);

    // Reset streaks to 0 for all identified users
