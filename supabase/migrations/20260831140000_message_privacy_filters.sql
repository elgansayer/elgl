...
ALTER TABLE public.users
  ADD COLUMN IF NOT EXISTS message_filters jsonb;

UPDATE public.users
   SET message_filters = '{"enabled":false,"allow_everyone":true}'::jsonb
 WHERE message_filters IS NULL;

ALTER TABLE public.users
  ALTER COLUMN message_filters SET DEFAULT '{"enabled":false,"allow_everyone":true}'::jsonb,
  ALTER COLUMN message_filters SET NOT NULL;
...
BEGIN
  ...
  -- Serialise first-message admission per room. Without this lock, concurrent
  -- inserts can both observe an empty conversation and bypass the boundary.
  PERFORM 1
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'message_sender_is_not_a_room_member'
      USING ERRCODE = 'P0001';
  END IF;
  ...
  -- Existing conversations remain usable, including a recipient's reply.
  -- Filters only gate the first message in a direct room.
  IF EXISTS (
    SELECT 1
  ...
  filtering_enabled := CASE
    ...
    ELSE filters ?| ARRAY[
      ...
      'same_native_language', 'same_target_language', 'same_gender', 'same_age',
      'sameNativeLanguage', 'sameTargetLanguage', 'sameGender', 'sameAge'
    ]
  END;

  allow_everyone := CASE
    WHEN filters ? 'allow_everyone' THEN COALESCE((filters ->> 'allow_everyone')::boolean, false)
  ...
  IF COALESCE(
       (filters ->> 'same_gender')::boolean,
       (filters ->> 'sameGender')::boolean,
       false
     )
  ...
  IF COALESCE(
       (filters ->> 'same_native_language')::boolean,
       (filters ->> 'sameNativeLanguage')::boolean,
       false
     )
     AND NOT EXISTS (
       SELECT 1
  ...
  IF COALESCE(
       (filters ->> 'same_target_language')::boolean,
       (filters ->> 'sameTargetLanguage')::boolean,
       false
     )
     AND NOT EXISTS (
       SELECT 1
  ...
  IF COALESCE(
       (filters ->> 'same_age')::boolean,
       (filters ->> 'sameAge')::boolean,
       false
     )
