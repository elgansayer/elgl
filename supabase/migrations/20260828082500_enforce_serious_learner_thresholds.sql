-- #1701: Keep location discovery aligned with the algorithmic Serious Learner
-- contract used by standard discovery.
--
-- This is a forward-only replacement of the existing function. Its signature,
-- result shape, validation and privileges remain unchanged for mixed-version
-- backends. Only the meaning of serious_only is corrected to use the documented
-- study streak and correction ratio thresholds.
...
BEGIN
    ...
    WHERE
        ...
        AND (
            NOT COALESCE(serious_only, false)
            OR (
                COALESCE(u.study_streak_days, 0) > 7
                AND COALESCE(u.correction_ratio, 0) >= 0.8
            )
        )
...
COMMENT ON FUNCTION public.search_nearby_users(
  ...
  'Service-role-only PostGIS discovery RPC. Validates inputs and applies bounded privacy-safe filters; serious_only requires study_streak_days > 7 and correction_ratio >= 0.8.';
