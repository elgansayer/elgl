...
5. The optional outer `AgentRouter` may route explicit phase-specific CLI providers only when `routing_enabled` is deliberately configured. It is not the retired swarm and is enabled by default.
