...
Changes to provider order, authentication, worker isolation, merge authority, generation ownership or durable-state schema must update the corresponding Factory architecture documentation and tests in the same pull request. Major dependency upgrades that alter any of those contracts require migration-specific acceptance criteria before autonomous merge.
