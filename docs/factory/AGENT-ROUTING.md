...
| OpenCode Go | `opencode-go/kimi-k3` | `opencode-go/qwen3.8-max` for planning, architecture, security, and review; `opencode-go/kimi-k2.7-code` for general action | Strongest available long-horizon coding model, a diverse reasoning model for review, and a high-capacity coding model for mechanical work |
