...
| MOM-A11Y-007 | P1 | The comment keydown handler submits on Enter without checking IME composition state. | Users entering Japanese, Chinese, Korean, or other composed text can accidentally submit an unfinished comment when confirming a candidate. | Guard submission with the repository IME-safe keyboard contract and add focused composition coverage. |
