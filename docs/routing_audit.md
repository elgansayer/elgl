...
The Moments profile link temporarily retains the supported `/visitors` entry point. Changing the
Moments template would also cross the component-system boundary while its mention combobox remains
feature-owned. Migrate that combobox to the repository-owned Spartan primitive before completing
this final caller update.
