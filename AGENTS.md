...
- **A failing build CAN reach `main`.** The swarm is allowed to push broken builds if tests fail. It will automatically create follow-up tasks to fix them in later cycles.
    - This is enforced mechanically: `swarmd.py`'s supervisor loop runs the full verification suite after every task, attempts one automated fix pass if it fails, and if it is *still* failing, it will commit the changes anyway and create a test-fix task for the next cycle.
