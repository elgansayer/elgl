...
- **A failing build MUST NOT reach `main`.** Every PR must pass the full verification suite before merge. The swarm must fix build errors and failing tests within the PR branch itself before merging. Do not create follow-up "fix" PRs for failures that should have been caught before merge.
  ...
  - The exact repository-owner user may manually bypass only the dedicated `factory/independent-review` ruleset, only through an existing pull request, and only after `CI / required` succeeds on the current head. The baseline pull-request and CI ruleset has no bypass actors. Broad role, team, app, deploy-key, direct-push, or always-mode bypasses remain banned. Manual use must be deliberate and auditable; the Factory service identity cannot use it.
