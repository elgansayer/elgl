...
from openhands_factory import architecture_guard, git_workflow, pipeline, verification
...
_ORIGINAL_COMMANDS_FOR = pipeline.commands_for
_ORIGINAL_BUILD_SYSTEM_PROMPT = pipeline.build_system_prompt
_ORIGINAL_PIPELINE_INIT = FactoryPipeline.__init__
_ORIGINAL_ADD_WORKTREE = GitWorkflow._add_worktree
_ORIGINAL_SINGLE_OWNER_GUARD = architecture_guard.assert_single_owner
_INSTALLED = False

_FORBIDDEN_AUTONOMOUS_TRIGGER = re.compile(
    r"(?m)^\s*(?:schedule|issues|pull_request|pull_request_target|workflow_run|"
...
def install_repository_profile() -> None:
    ...
    pipeline.FactoryPipeline.__init__ = _profiled_pipeline_init
    git_workflow.GitWorkflow._add_worktree = _profiled_add_worktree
