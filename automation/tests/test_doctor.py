...
def test_doctor_reports_openai_subscription_credentials(
    ...
    monkeypatch.setattr("openhands_factory.doctor.subprocess.run", lambda *args, **kwargs: CompletedProcess(args, 0, "", ""))
...
def test_health_reports_stale_daemon_heartbeat(tmp_path: Path) -> None:
    factory_config = config(tmp_path)
    now = datetime.now(UTC)
    atomic_write_json(factory_config.state_dir / "daemon.json", {"status": "running", "updated_at": (now - timedelta(minutes=5)).isoformat(), "active_jobs": ["242"]})
...
def test_health_rejects_a_materially_future_dated_heartbeat(tmp_path: Path) -> None:
    factory_config = config(tmp_path)
    now = datetime.now(UTC)
    atomic_write_json(factory_config.state_dir / "daemon.json", {"status": "running", "updated_at": (now + timedelta(minutes=5)).isoformat(), "active_jobs": []})
    check = daemon_health_check(factory_config, now)
    assert not check.passed
...
def test_health_reports_no_pull_request_progress_with_active_jobs(tmp_path: Path) -> None:
    factory_config = config(tmp_path)
    now = datetime.now(UTC)
    atomic_write_json(factory_config.state_dir / "daemon.json", {"status": "running", "updated_at": now.isoformat(), "active_jobs": ["1"]})
    JobStore(factory_config.state_dir / "jobs.json").save({"1": Job(Task("1", "Task", "", "github", 0), JobState.IMPLEMENTING)})
