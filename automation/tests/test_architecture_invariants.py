...
def test_all_factory_phases_share_the_openhands_conversation_runner() -> None:
    ...
    # Implementation, security review, quality repair, CI repair, independent review,
    # and architect work all flow through the same OpenHands ConversationRunner.
...
def test_generation_and_architecture_contracts_are_documented() -> None:
    architecture = (ROOT / "docs/factory/ACTIVE_ARCHITECTURE.md").read_text(encoding="utf-8")
    generation = (ROOT / "docs/factory/GENERATION_OWNERSHIP.md").read_text(encoding="utf-8")

    assert "OpenHands Factory" in architecture
    assert "OpenCode Go" in architecture
    assert "retired" in architecture.lower()
    assert "durable-state schema" in generation
    assert "fails closed" in generation
