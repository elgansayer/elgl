...
| `accent`              | `#E14F42` (raspberry-ember, 50-700 scale)                                              | same (static, not theme-branched) | Reserved for celebratory/gift moments — distinct from the primary/secondary duet so it reads as a special occasion.                                                                                              |
...
4.5:1 (WCAG AA) in both themes. `accent` and `neon` are intentionally left as static (non-branched)
scales for now — they're decorative/tint use, lower contrast-criticality — flagged as a Phase 2
follow-up if a specific use case needs per-theme tuning.
